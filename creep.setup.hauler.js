...
setup.maxCount = function(room){
    ...
    if(room.population && room.population.typeCount['miner'] > 0) {
        ...
        if( room.links.length < 3 || (room.storage && room.storage.energy > MAX_STORAGE_ENERGY && room.containerController && _.sum(room.containerController, 'store.energy') == 0 ) count++;
    }
    return count; 
};
