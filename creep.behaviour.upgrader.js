module.exports = {
    ...
    run: function(creep) {
        if( creep.room.containerController ){                
            ...
            let contRange = creep.pos.getRangeTo(creep.room.containerController);
            ...
            if( contRange < 2 && creep.carry.energy <= (creep.data.body&&creep.data.body.work ? creep.data.body.work :  (creep.carryCapacity/2) )) 
                creep.withdraw(creep.room.containerController, RESOURCE_ENERGY);
            if( controllerRange < 4 ) 
                creep.upgradeController(creep.room.controller);
            if( contRange > 0 ) 
                Creep.action.upgrading.drive(creep, creep.room.containerController.pos, Infinity);
        } else {
