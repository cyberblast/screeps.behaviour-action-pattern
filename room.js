...
mod.extend = function(){
    ...
    Room.prototype.processNeutrals = function() {
        ...
        const registerNeutral = creep => {
            if (!this.memory.neutralIDs.includes(creep.id)) {
    ...
    let run = (memory, roomName) => {
        try {
            ...
            if (room) { // has sight
                ...
                room.goneAlly.forEach(id => Room.goneAlly.trigger(id));
                room.goneNeutral.forEach(id => Room.goneNeutralPlayer.trigger(id));
                room.allyIDs.forEach(id => Room.knownAlly.trigger(id));
                room.neutralIDs.forEach(id => Room.knownNeutralPlayer.trigger(id));
                room.newAlly.forEach(creep => Room.newAlly.trigger(creep));
                room.newNeutral.forEach(creep => Room.newNeutralPlayer.trigger(creep));
            } else {
                if (memory.allyIDs) _.forEach(memory.allyIDs, id => Room.knownAlly.trigger(id));
                if (memory.neutralIDs) _.forEach(memory.neutralIDs, id => Room.knownNeutralPlayer.trigger(id));
            }
        } catch (e) {
