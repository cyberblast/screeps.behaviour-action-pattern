var work = {
    run: function(creep) {
        ...
        if( creep.room.controller.my && creep.room.situation.invasion )
            assignment = MODULES.creep.assignActionWithTarget(creep, MODULES.creep.action.healer.defending);
        else {
            var flag = _.find(Game.flags, {'color': FLAG_COLOR.destroy }) || _.find(Game.flags, {'color': FLAG_COLOR.invade });
            if( flag ) assignment = MODULES.creep.assignActionWithTarget(creep, MODULES.creep.action.healer.invading);
